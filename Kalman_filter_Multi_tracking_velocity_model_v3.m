...
if images_folder == 0
error('Operation canceled. Program stopped.');
end
...
% Sort by file name number Assuming file names are numeric
...
error('No jpg images found in the entered folder.');
end
...
% Prepare CSV file
% ===========================
csv_path = fullfile(output_folder, 'Kalman_filter_veloc_trajectories.csv');
    ...
    if ~isempty(current_detections)
        if isempty(all_x_predict)
            ...
            for d = 1:size(current_detections,1)
                ...
                X0 = [det_center_x;initial_x_dot;det_center_y;initial_y_dot];
